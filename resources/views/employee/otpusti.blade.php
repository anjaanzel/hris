    ...
    <div class="container">
        <div class="row">
            <div class="card col s12 m12 l12 xl12 mt-20">
                <div>
                <h4 class="center grey-text text-darken-2 card-title">Prekid radnog odnosa</h4>
                </div>
                <hr>
                <div class="card-content">
                    <form action="{{route('employees.prebaciUEx',$employee->id)}}" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <i class="material-icons prefix">wc</i>
                                <input type="text" name="pol" id="pol" value="{{old('pol') ? : $employee->pol}}">
                            ...
                            <div class="input-field col s12 m6 l6 xl8 offset-xl2">
                                <i class="material-icons prefix">date_range</i>
                                <input type="text" name="datum_odlaska" id="datum_odlaska" class="datepicker" value="{{Request::old('datum_odlaska') ? : $employee->datum_odlaska}}">
                                <label for="datum_odlaska">Datum odlaska</label>
                                <span class="{{$errors->has('datum_odlaska') ? 'helper-text red-text' : ''}}">{{$errors->has('datum_odlaska') ? $errors->first('datum_odlaska') : '' }}</span>
                            </div>
                            ...
                            <div class="input-field col s12 m12 l12 xl8 offset-xl2">
                                <i class="material-icons prefix">add_location</i>
                                <textarea name="razlog_odlaska" id="razlog_odlaska" class="materialize-textarea" >{{Request::old('razlog_odlaska') ? : $employee->razlog_odlaska}}</textarea>
                                <label for="razlog_odlaska">Razlog odlaska</label>
                                <span class="{{$errors->has('razlog_odlaska') ? 'helper-text red-text' : ''}}">{{$errors->has('razlog_odlaska') ? $errors->first('razlog_odlaska') : ''}}</span>
                            </div>
                        ...
                        @csrf()
                        <div class="row">
                            <button type="submit" class="btn waves-effect waves-light col s8 offset-s2 m4 offset-m4 l4 offset-l4 xl4 offset-xl4">Sačuvaj bivšeg zaposlenog</button>
                        </div>
                    </form>
                </div>
